...
ck = "90915d2163323f6b8d0542edf648e3e3"  # 本地环境ck，环境变量存在此处不生效
...
class PGSH:
    ...
    def sg(self, y):
        ...
        data = f"appSecret=nFU9pbG8YQoAe1kFh+E7eyrdlSLglwEJeA0wwHB1j5o=&channel=android_app&timestamp={timestamp}&token={self.token}&version=1.59.6&{path}"
        ...
        try:
            ...
            if r['code'] == 0:
                ...
                try:
                    ...
                    coin_code = r['code']
                    balance = r['data']['integral'] if coin_code == 0 else 'N/A'
                    # print(f"[{phone}] ✅登录成功！积分余额: {balance}")
                    print(f"[{phone}] ✅今日获得积分: {total_amount}，总余额: {balance}")
